...
y=int16.empty(1000,0);
z=int16.empty(1000,0);
Fz=double.empty(100000,0);
Fy=double.empty(100000,0);
Fx=double.empty(100000,0);
tempo=int16.empty(10000,0);
tempo(1)=0;
t=0;
...
if (ports.getLastSize()>0),             % Is there at least 1 available port?
    ...
    if (isOpen==1),
        ...
        while (elapsed_time<30 && output.size>=0 ),        % Loop for 10sec (quit if any error)
...
subplot(3,1,1)
plot(tempo(1:t),z,1:ze,1000*Fz(1:ze));
legend('z (Ard)', 'z (Opto)');
hold on;

subplot(3,1,2)
plot(tempo(1:t),y,1:ze,1000*Fy(1:ze));
legend('y (Ard)', 'y (Opto)');
hold on;

subplot(3,1,3)
plot(tempo(1:t),x,1:ze,1000*Fx(1:ze));
legend('x (Ard)', 'x (Opto)');
